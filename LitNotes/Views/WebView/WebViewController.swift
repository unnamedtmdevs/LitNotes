...
class WebViewController: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    @AppStorage("saved_url") var savedUrl: String = ""
    ...
    private func getRequest() {
        guard let url = URL(string: ServerConfig().server) else { return }
        self.url_link = url
        self.getInfo()
    }
    ...
    private func getInfo() {
        ...
        if savedUrl == "about:blank" || savedUrl.isEmpty {
            request = URLRequest(url: self.url_link)
        } else {
            if let currentURL = URL(string: savedUrl) {
    ...
    private func setupWebView() {
        let urlString = savedUrl.isEmpty ? url_link.absoluteString : savedUrl
    ...
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        loadCheckTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: false) { [weak self] _ in
            if let strongSelf = self, !strongSelf.isPageLoadedSuccessfully {
                print("⏱ Page didn't load within 5 seconds")
            }
        }
    }
    ...
    func saveCookie() {
        ...
        if let cookies = cookieJar.cookies {
            ...
            UserDefaults.standard.set(data, forKey: "saved_cookies")
        }
    }
    ...
    func loadCookie() {
        ...
        if let data = ud.object(forKey: "saved_cookies") as? Data, let cookies = NSKeyedUnarchiver.unarchiveObject(with: data) as? [HTTPCookie] {
